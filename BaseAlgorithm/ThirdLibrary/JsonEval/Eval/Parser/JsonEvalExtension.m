...
@implementation JsonEvalExtension
...
{
    _registeredStruct[defineDict[@"name"]]=defineDict;
}
...
break;
            ...
            case '{': {
                ...
                if (end != NSNotFound) {
                    ...
                    NSDictionary *subStructDefine = [JsonEvalExtension registeredStruct][subStructName];
...
static NSString *extractStructName(NSString *typeEncodeString)
{
    ...
    for (int i = 0; i< typeString.length; i++) {
        char ccc = [typeString characterAtIndex:i];
        if (ccc == '{' || ccc=='_') {
            firstValidIndex++;
        }else {
            break;
