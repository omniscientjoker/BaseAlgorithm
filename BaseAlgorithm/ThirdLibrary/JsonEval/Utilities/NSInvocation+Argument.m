...
- (void)jsonEval_setArgument:(id)obj atIndex:(NSInteger)argumentIndex {
...
#define set_with_args_struct(_dic_, _struct_, _param_, _key_, _sel_) \
    do { \
        if ([_dic_ isKindOfClass:[NSDictionary class]] && [_dic_.allKeys containsObject:_key_]) { \
    ...
    switch (*type) {
        ...
        case '*': // char *
        {
            ...
            if ([arg isKindOfClass:[NSString class]]) {
                const void *chr = [arg UTF8String];
                [self setArgument:&chr atIndex:index];
            }
        } break;
    ...
    NSAssert(supportedType, @"arg unsupportedType");
}

- (id)jsonEval_argumentAtIndex:(NSUInteger)index {
...
- (void)setArguments:(NSArray *)arguments {
    for (int index = 0; index < arguments.count; index++) {
        [self jsonEval_setArgument:arguments[index] atIndex:index];
    }
}
...
- (NSArray *)block_arguments {
    return [self arguments_startFromIndex:1];
}
...
#define WRAP_AND_RETURN(type) { type val = 0; [self getArgument:&val atIndex:(NSInteger)index]; return @(val); }
